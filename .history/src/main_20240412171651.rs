        ...
        loop {
            match self.tokens.get(self.program_cursor) {
                Some(tok) => match tok.kind {
                    ...
                    K::Jmp => {
                        let new_cursor = match self.jump_stack.pop() {
                            Some(c) => c,
                            None => panic!("fuck"),
                        };
                        self.jump_stack.push(self.program_cursor);
                        if self.mem[self.mem_cursor] == 0 {
                            self.program_cursor = new_cursor;
                        }
                    }
                    K::Pmj => {
                        let new_cursor = match self.jump_stack.pop() {
