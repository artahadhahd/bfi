use std::{
    collections::HashMap, env, fs, io::{self, Read, Write}, path::PathBuf, str::Chars
};
                ...
                Some(tok) => match tok.kind {
                    ...
                    K::Jmp => {
                        if self.mem[self.mem_cursor] == 0 {
                            self.program_cursor = *self.jmp_table.get(&self.program_cursor).unwrap();
                        }
                    }
                    K::Pmj => {
                        if self.mem[self.mem_cursor] != 0 {
...
fn main() {
    ...
    //     return;
    // } else if alen == 2 {
    //     let lexer = Lexer::from(include_str!("../test"));
    //     Interpreter::from(lexer).interpret();
    // }
    match alen {
        1 => launch_repl(),
        2 => {
            // let source = fs::read_to_string(PathBuf::from(args.nth(1).unwrap()));
            let source = fs::read_to_string(args.nth(1).unwrap());
        }
